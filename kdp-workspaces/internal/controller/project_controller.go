...
func (r *ProjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	createdInfo, err := kcpClient.CreateWorkspace(ctx, workspaceName, project.Namespace, project.Name)
	if err != nil {
...
func (r *ProjectReconciler) updateWorkspaceStatus(ctx context.Context, project *metav1.PartialObjectMetadata, info *kcp.WorkspaceInfo) error {
	// Fetch fresh copy of the project to avoid conflicts
	freshProject := &metav1.PartialObjectMetadata{}
	freshProject.SetGroupVersionKind(schema.GroupVersionKind{
		Group:   "maintainer-d.cncf.io",
		Version: "v1alpha1",
		Kind:    "Project",
	})
	...
	}, freshProject); err != nil {
		return fmt.Errorf("failed to get fresh project: %w", err)
	}

	// Create patch from fresh copy BEFORE making changes
	patch := client.MergeFrom(freshProject.DeepCopy())

	// Update annotations with workspace details
	if freshProject.Annotations == nil {
		freshProject.Annotations = make(map[string]string)
	}
	freshProject.Annotations[AnnotationWorkspaceName] = info.Name
	freshProject.Annotations[AnnotationWorkspaceURL] = info.URL
	freshProject.Annotations[AnnotationWorkspacePhase] = info.Phase

	// Apply the patch
	if err := r.Patch(ctx, freshProject, patch); err != nil {
	...
	condition := metav1.Condition{
		Type:               WorkspaceReadyCondition,
		ObservedGeneration: freshProject.Generation,
	...
	return r.updateCondition(ctx, freshProject, condition)
}
...
func (r *ProjectReconciler) updateCondition(ctx context.Context, project *metav1.PartialObjectMetadata, condition metav1.Condition) error {
	...
	// Create patch from the current state BEFORE making changes
	patch := client.MergeFrom(currentProject.DeepCopy())

	// Extract conditions from status if they exist
	var conditions []metav1.Condition
	if statusMap, ok := currentProject.Object["status"].(map[string]any); ok {
	...
	// Convert conditions slice to []any for unstructured storage
	// Each condition must be a map[string]any to avoid deep copy errors
	conditionsInterface := make([]any, len(conditions))
	for i, cond := range conditions {
		conditionsInterface[i] = map[string]any{
			"type":               cond.Type,
	...
	// Update the status in the unstructured object
	statusMap := make(map[string]any)
	if status, ok := currentProject.Object["status"].(map[string]any); ok {
		statusMap = status
	}
	...
	if err := r.Status().Patch(ctx, currentProject, patch); err != nil {
