...
// CreateWorkspace creates a new workspace in KCP
func (c *Client) CreateWorkspace(ctx context.Context, orgName, namespace, resourceName string) (*WorkspaceInfo, error) {
	if orgName == "" {
		return nil, fmt.Errorf("organization name cannot be empty")
	}

	workspaceType := c.config.WorkspaceType
	if workspaceType == "" {
	...
	workspace := &tenancyv1alpha1.Workspace{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				"organization-name":      resourceName,
			},
		},
	...
	// Get workspace path from config (default "root")
	workspacePath := logicalcluster.NewPath(c.config.WorkspacePath)

	clusterClient := c.GetClusterClient()
	createdWs, err := clusterClient.Cluster(workspacePath).TenancyV1alpha1().Workspaces().Create(
		ctx, workspace, metav1.CreateOptions{},
	)
	if err != nil {
		return nil, fmt.Errorf("failed to create workspace %s: %w", orgName, err)
	}
...
func (c *Client) GetWorkspace(ctx context.Context, workspaceName string) (*WorkspaceInfo, error) {
	...
	ws, err := clusterClient.Cluster(workspacePath).TenancyV1alpha1().Workspaces().Get(
		ctx, workspaceName, metav1.GetOptions{},
	)
	if err != nil {
...
func (c *Client) DeleteWorkspace(ctx context.Context, workspaceName string) error {
	...
	err := clusterClient.Cluster(workspacePath).TenancyV1alpha1().Workspaces().Delete(
		ctx, workspaceName, metav1.DeleteOptions{},
	)
	if err != nil {
		if errors.IsNotFound(err) {
			return nil
...
func (c *Client) WaitForWorkspaceReady(ctx context.Context, workspaceName string) (*WorkspaceInfo, error) {
	...
	err := wait.PollUntilContextTimeout(ctx, WorkspaceCheckInterval, WorkspaceReadyTimeout, true,
	...
	if err != nil {
		if lastInfo != nil {
			return lastInfo, fmt.Errorf("workspace %s not ready after timeout, current phase: %s: %w",
				workspaceName, lastInfo.Phase, err)
		}
