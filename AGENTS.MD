...
- Deployment uses plain Kubernetes manifests under `deploy/manifests` (production only). No Kustomize and no Helm.
...
- Verify Secrets exist and the image tag is pullable from GHCR.
